...
for ii = 1:size(CartInputPoints,1)
     ...
     plot(Lidar(1).beam(ii).CartX/R, Lidar(1).beam(ii).CartY/R,'b.','LineWidth',1.2)
     plot(Lidar(2).beam(ii).CartX/R, Lidar(2).beam(ii).CartY/R,'r.','LineWidth',1.2)
     
     % plot the focal points
     plot(CartInputPoints(ii,1)/R,CartInputPoints(ii,2)/R,'*')
...
title('Staring mode in xy-plane','Interpreter','Latex')
axis equal
%% End of calculations. Plotting of the errors

% plot the error vs the azimuthal angle
PhiAngleDiff = abs(rad2deg(Lidar(1).phi - Lidar(2).phi));
PhiAngleDiff(PhiAngleDiff > 180) = 360 - PhiAngleDiff(PhiAngleDiff > 180);
ThetaAngle = rad2deg(Lidar(1).theta);

uAbsError = abs(ReconstrVelVector(1,:) - umag);
vAbsError = abs(ReconstrVelVector(2,:) - vmag);

uAbsErrorMat = reshape(uAbsError,[length(yPointsMat1),length(yPointsMat1)]);
vAbsErrorMat = reshape(vAbsError,[length(yPointsMat1),length(yPointsMat1)]);

%%
figure
plot3(Lidar(1).x/R,Lidar(1).y/R,Lidar(1).z/R,'b^','LineWidth',1.2)
hold on
plot3(Lidar(2).x/R,Lidar(2).y/R,Lidar(2).z/R,'rd','LineWidth',1.2)
for ii = 1:size(CartInputPoints,1)
     
     % plot the beam of each LIDAR
     plot3(Lidar(1).beam(ii).CartX/R, Lidar(1).beam(ii).CartY/R,...
         Lidar(1).beam(ii).CartZ/R,'b.')
     plot3(Lidar(2).beam(ii).CartX/R, Lidar(2).beam(ii).CartY/R,...
         Lidar(2).beam(ii).CartZ/R, 'r.')
     
     % plot the focal points
     plot3(CartInputPoints(ii,1)/R,CartInputPoints(ii,2)/R,CartInputPoints(ii,3)/R,'*')
end
hold off
grid on
grid minor
...
PhiAngleDiff2 = reshape(PhiAngleDiff,[length(yPointsMat1),length(yPointsMat1)])';
ThetaAngle2 = reshape(ThetaAngle,[length(yPointsMat1),length(yPointsMat1)]);
ThetaAngle2 = ThetaAngle2';

figure
surf(PhiAngleDiff2,ThetaAngle2,uAbsErrorMat')
xlabel('$\Delta\phi$ [$^\circ$]','Interpreter','Latex')
ylabel('$\theta$ [$^\circ$]','Interpreter','Latex')
zlabel('Normalized u-error [-]','Interpreter','Latex')
view(35,31)
xlim([0 265])
set(gca,'fontsize', 14)

figure
surf(PhiAngleDiff2,ThetaAngle2,vAbsErrorMat')
xlabel('$\Delta\phi$ [$^\circ$]','Interpreter','Latex')
ylabel('$\theta$ [$^\circ$]','Interpreter','Latex')
zlabel('Normalized v-error [-]','Interpreter','Latex')
view(35,31)
set(gca,'fontsize', 14)
